...
export default function Profile() {
  ...
  const onDelete = async () => {
    if (!confirm('Delete your account? This is reversible only by admins.')) return;
    try {
        ...
        <div>
          ...
          <div className="mb-2">
            {preview ? (
              ...
              <div className="w-24 h-24 rounded-full bg-gray-100 flex items-center justify-center">No avatar</div>
            )}
          </div>
          <input type="file" accept="image/*" onChange={(e) => setFile(e.target.files?.[0] || null)} />
          {preview && (
            <div className="mt-2">
              <button type="button" className="btn-secondary mr-2" onClick={async () => {
