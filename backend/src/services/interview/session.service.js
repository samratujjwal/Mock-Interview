...
import { appendAnswerToMemory, appendQuestionToMemory, buildInitialSessionMemory } from './contextMemory.service.js';
...
export async function createInterviewSession(userId, data = {}) {
  ...
  const hydratedMemory = initialQuestions.reduce((memory, question) => appendQuestionToMemory(memory, question), initialMemory);
...
export async function listInterviewSessionsByUser(userId, { page = 1, limit = 10 } = {}) {
...
export async function updateInterviewSession(userId, sessionId, updates = {}) {
  if (!isValidObjectId(sessionId)) return null;
  const allowedUpdates = ['title', 'role', 'type', 'difficulty', 'companyMode', 'status', 'memory', 'currentQuestionIndex', 'summary', 'totalScore', 'startedAt', 'completedAt'];
  const payload = {};
  ...
  return InterviewSession.findOneAndUpdate({ _id: sessionId, userId }, { $set: payload }, { new: true }).lean().exec();
}

export async function addQuestionToSession(userId, sessionId, questionData = {}) {
  if (!isValidObjectId(sessionId)) return null;
  ...
  const currentSession = await InterviewSession.findOne({ _id: sessionId, userId }).lean().exec();
  const nextMemory = appendQuestionToMemory(currentSession?.memory || {}, question);
...
export async function submitInterviewAnswer(userId, sessionId, questionId, answerData = {}) {
  if (!isValidObjectId(sessionId) || !questionId) return null;

  const currentSession = await InterviewSession.findOne({ _id: sessionId, userId }).lean().exec();
  const question = (currentSession?.questions || []).find((entry) => String(entry.questionId) === String(questionId));
  const submittedAt = answerData.submittedAt || new Date();

  const answerPayload = {
    'questions.$.answer.response': String(answerData.response || '').trim() || null,
    'questions.$.answer.submittedAt': submittedAt,
    'questions.$.answer.score': Number.isFinite(Number(answerData.score)) ? Number(answerData.score) : null,
    'questions.$.answer.feedback': String(answerData.feedback || '').trim() || null,
    'questions.$.answer.aiEvaluation': String(answerData.aiEvaluation || '').trim() || null,
    'questions.$.answer.responseTimeMs': Number.isFinite(Number(answerData.responseTimeMs)) ? Number(answerData.responseTimeMs) : null,
    'questions.$.answer.thinkingTimeMs': Number.isFinite(Number(answerData.thinkingTimeMs)) ? Number(answerData.thinkingTimeMs) : null,
    'questions.$.answer.confidence': Number.isFinite(Number(answerData.confidence)) ? Number(answerData.confidence) : null,
    'questions.$.answer.evaluatedAt': answerData.evaluatedAt || null,
    'questions.$.status': 'answered',
  };

  const updatedMemory = appendAnswerToMemory(currentSession?.memory || {}, question || {}, {
    ...answerData,
    submittedAt,
  });
